"""p3_csv_pipeline.py - CSV ETVL pipeline, case_csv_pipeline.py duplicated and the duplicate (this file) modified by Caleb Owens to include calculations of additional statistics (median, mode) for a .csv file listing ACT scores instead of happiness scores
...
def verify_stats(*, stats: dict[str, float]) -> None:
    ...
    Returns:
        None
    """
    required = {"count", "min", "max", "mean", "median", "mode", "stdev"}
...
def run_csv_pipeline(*, raw_dir: Path, processed_dir: Path, logger: Any) -> None:
    ...
    input_file = raw_dir / "act_test_scores.csv"
    output_file = processed_dir / "act_test_scores_stats.txt"

    # E
    scores = extract_csv_scores(file_path=input_file, column_name="ACT Score")
